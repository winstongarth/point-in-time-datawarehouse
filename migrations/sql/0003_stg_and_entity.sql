...
-- is deliberate: this table is truncated and rebuilt on every
-- parse run, duplicates are expected until promoted to core, and
...
-- but the ticker mapping genuinely is bitemporal (tickers get
-- reassigned), so it gets the same non-overlap rigor as invariant 1 does
-- for fundamental_fact/price_fact - just applied to (ticker, knowledge
